...
void getWindowSize(vec2 output)
{
    output[0] = (float)renderState.windowWidth;
    output[1] = (float)renderState.windowHeight;
}
...
{
    renderState.defaultShader = createShader("src/shaders/default.frag.glsl", "src/shaders/default.vert.glsl");
    renderState.circleShader = createShader("src/shaders/circle.frag.glsl", "src/shaders/circle.vert.glsl");

    mat4x4_ortho(renderState.projection, 0, renderState.windowWidth, 0, renderState.windowHeight, -2, 2);

    glUseProgram(renderState.defaultShader);
    glUniformMatrix4fv(glGetUniformLocation(renderState.defaultShader, "projection"), 1, GL_FALSE,
                       &renderState.projection[0][0]);

    glUseProgram(renderState.circleShader);
    glUniformMatrix4fv(glGetUniformLocation(renderState.circleShader, "projection"), 1, GL_FALSE,
                       &renderState.projection[0][0]);
}
...
void initImageTexture(ImageTexture *imageTexture, const char *path)
{
    ...
    imageTexture->textureData =
        stbi_load(path, &imageTexture->width, &imageTexture->height, &imageTexture->numChannels, 0);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, imageTexture->width, imageTexture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 imageTexture->textureData);
...
{
    renderQuad(renderState.defaultShader, renderState.defaultTexture, sprite->rotation, sprite->position, sprite->size,
               COLOR_WHITE);
}
...
void drawCircle(const float radius, const vec2 pos)
{
    ...
    renderQuad(renderState.circleShader, renderState.defaultTexture, 0.0f, pos3, (vec2){radius * 2, radius * 2},
               COLOR_WHITE);
}
