...
void initShaders(void) {
  renderState.defaultShader = createShader("src/shaders/default.frag.glsl",
                                           "src/shaders/default.vert.glsl");
  renderState.circleShader = createShader("src/shaders/circle.frag.glsl",
                                          "src/shaders/circle.vert.glsl");
...
void renderQuad(int shader, int texture, const float rotation, const vec3 pos,
...
void drawSprite(Sprite *sprite) {
  renderQuad(renderState.defaultShader, renderState.defaultTexture,
             sprite->rotation, sprite->position, sprite->size, COLOR_WHITE);
}

void drawCircle(const float radius, const vec2 pos) {
  vec3 pos3 = {pos[0], pos[1], 0.0f};
  renderQuad(renderState.circleShader, renderState.defaultTexture, 0.0f, pos3,
             (vec2){radius * 2, radius * 2}, COLOR_WHITE);
}
